...
function List-AmazonS3Buckets ($AccessKeyId, $SecretAccessKey) {
...
function List-AmazonS3BucketObjects ($AccessKeyId, $SecretAccessKey, $Bucket) {
    $timeStamp = [System.DateTime]::UtcNow;
    $strToSign = [System.String]::Format("GET`n`n`n{0}`n/{1}/", $timeStamp.ToString("ddd, dd MMM yyyy HH:mm:ss 'GMT'"), $Bucket);
    $utf8 = New-Object System.Text.UTF8Encoding;
    $hasher = New-Object System.Security.Cryptography.HMACSHA1;
    $hasher.Key = $utf8.GetBytes($SecretAccessKey);
    $req = [System.Net.HttpWebRequest][System.Net.WebRequest]::Create([System.String]::Format("http://{0}.s3.amazonaws.com/", $Bucket));
    $req.Headers.Add("Authorization", [System.String]::Format("AWS {0}:{1}", $AccessKeyId, [System.Convert]::ToBase64String($hasher.ComputeHash($utf8.GetBytes($strToSign)))));
    ([System.Net.HttpWebRequest]$req).Date = $timeStamp;
    try {
        foreach($xmlObj in ([xml](New-Object System.IO.StreamReader -arg ([System.Net.HttpWebRequest]$req).GetResponse().GetResponseStream()).ReadToEnd()).ListBucketResult.Contents) {
            $obj = New-Object psobject -Property @{
                Key = $xmlObj.Key;
                Size = $xmlObj.Size;
                LastModified = [System.DateTime]::Parse($xmlObj.LastModified);
            };
            $obj;
        }
    }
